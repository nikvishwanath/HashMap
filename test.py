...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        # If the given key already exists in the HashMap, remove it so that the value can 
        # be updated.  
        if self.contains_key(key):
            self.remove(key)
        # Insert and update the size of the HashMap.  
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        for i in range(self._buckets.length()):
            ...
            for i in node:
                index = self._hash_function(i.key) % array_capacity
                da.get_at_index(index).insert(i.key, i.value)
...
def find_mode(da: DynamicArray) -> (DynamicArray, int):
    ...
    # Iterate through the HashMap to find the key(s) with the highest frequency.  
    for bucket in range(map._buckets.length()):
        nodes = map._buckets.get_at_index(bucket)
        for node in nodes:
            # Add the current key to the return DynamicArray if it shares a frequency with the current mode.   
            if node.value == current_mode:
                return_da.append(node.key)
        
    return(return_da, current_mode)


m = HashMap(7, hash_function_2)
m.put("1", 1)
m.put("2", 1)
m.remove("1")
